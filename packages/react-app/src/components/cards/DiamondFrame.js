    ...
    return (
        <div className="playerCard noselect" style={{backgroundImage: `url(${Frame.frameIdToString(props.player.frame)})`}}>
            ...
            <div className="card-info-name player-info">
                <label htmlFor="file-diamond">XP&nbsp;</label>
                <progress id="file-diamond" max={GameContract.getXpRequireToLvlUp(props.player.score)}
                          value={props.player.xp}/>
            </div>
            <div className="card-info-name player-info">
                <label htmlFor="fileStamina"><BsFillLightningChargeFill
                    style={{color: 'yellow'}}/></label>
                <progress id="fileStamina" max="100" value={props.stamina}/>
            </div>
        </div>
    );
}
