    ...
    return (
        <div className="playerCard noselect" style={{backgroundImage: `url(${Frame.frameIdToString(props.player.frame)})`}}>
            ...
            <div className="card-info-name player-info">
                <label htmlFor="file-silver">XP&nbsp;</label>
                <progress id="file-silver" max={GameContract.getXpRequireToLvlUp(props.player.score)}
                          value={props.player.xp}/>
            </div>
            <div className="card-info-name player-info">
                <label htmlFor="fileStamina-silver"><BsFillLightningChargeFill
                    style={{color: 'yellow'}}/></label>
                <progress id="fileStamina-silver" max="100" value={props.stamina}/>
            </div>
        </div>
    );
}
